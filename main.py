...
try:
    ...
    print(f"⚠️  Training functionality not available: {e}")
...
try:
    ...
    print(f"⚠️  Evaluation functionality not available: {e}")
...
try:
    ...
    print(f"⚠️  Video splitter functionality not available: {e}")
...
try:
    ...
    print(f"⚠️  Watermark remover functionality not available: {e}")
...
class DataProcessingToolkit:
    ...
    def show_main_menu(self):
        ...
        print("\n" + "="*60)
        print("🛠️  DATA PROCESSING TOOLKIT")
        print("="*60)
        print("Choose an operation:")
        print("1. 📄 Convert format (Supervisely → YOLO → Darknet)")
        print("2. 🏋️  Train/evaluate datasets")
    ...
    def format_conversion_menu(self):
        ...
        while True:
            ...
            print("4. ⚙️  Settings (Default folders)")
    ...
    def format_conversion_settings(self):
        ...
        print("\n" + "="*50)
        print("⚙️  FORMAT CONVERSION SETTINGS")
        print("="*50)
        ...
        print(f"📂 Default input folder:  {current_input}")
        print(f"📁 Default output folder: {current_output}")
        ...
        while True:
            try:
                ...
                print("\n👋 Returning to format conversion menu...")
                break
    ...
    def change_default_input_folder(self):
        ...
        print(f"\n📂 Current default input folder: {current}")
        
        new_folder = input("Enter new default input folder path: ").strip()
        if not new_folder:
            print("❌ No folder specified. Keeping current setting.")
            return
        
        # Optional: Check if folder exists and warn if it doesn't
        if not Path(new_folder).exists():
            confirm = input(f"⚠️  Folder '{new_folder}' doesn't exist. Set anyway? (y/N): ").strip().lower()
            if confirm != 'y':
                print("❌ Operation cancelled.")
                return
        
        if self.settings_manager.set_default_input_folder(new_folder):
            print(f"✅ Default input folder updated to: {new_folder}")
        else:
    ...
    def change_default_output_folder(self):
        ...
        print(f"\n📁 Current default output folder: {current}")
        ...
        if self.settings_manager.set_default_output_folder(new_folder):
            print(f"✅ Default output folder updated to: {new_folder}")
        else:
            print("❌ Failed to save settings.")
    ...
    def reset_settings_to_defaults(self):
        ...
        if confirm == 'y':
            ...
            print("📂 Input folder:  dataset_supervisely_format")
            print("📁 Output folder: dataset_yolo_format")
        else:
    ...
    def supervisely_to_yolo(self):
        ...
        print("\n" + "="*50)
        print("📄 SUPERVISELY → YOLO CONVERSION")
        print("="*50)
        
        # Get input directory using settings
        default_input = self.settings_manager.get_default_input_folder()
        input_dir = input(f"📂 Provide folder path where datasets exist (default: {default_input}): ").strip()
        if not input_dir:
        ...
        if not Path(input_dir).exists():
            ...
            print("💡 Tip: Use option 4 (Settings) to change default folders.")
            return
        ...
        try:
            ...
            if successful_datasets:
                ...
                print(f"✅ Created {len(successful_datasets)} YOLO dataset(s)")
    ...
    def train_evaluate_menu(self):
        ...
        if not TRAINING_AVAILABLE:
            print("\n" + "="*50)
            print("🏋️  TRAIN/EVALUATE DATASETS")
            print("="*50)
            print("❌ Training functionality is not available!")
        ...
        while True:
            ...
            print("5. ⚙️  Training settings (Coming soon)")
    ...
    def train_yolo_datasets(self):
        ...
        try:
            ...
            print(f"\n📋 TRAINING PLAN:")
            print(f"📊 Datasets to train: {', '.join(map(str, dataset_identifiers))}")
            ...
            for i, dataset_identifier in enumerate(dataset_identifiers):
                try:
                    ...
                    if not dataset_path.exists():
                        print(f"❌ Dataset {dataset_identifier} not found at {dataset_path}")
                        continue
                    ...
                    print(f"\n❌ Unexpected error training dataset {dataset_identifier}: {e}")
                    continue
    ...
    def get_dataset_identifiers_for_training(self):
        ...
        while True:
            try:
                ...
                if not user_input:
                    print("❌ Please enter at least one dataset identifier.")
                    continue
                ...
                if not dataset_identifiers:
                    print("❌ Please enter valid dataset identifiers.")
                    continue
                ...
                for identifier in unique_identifiers:
                    ...
                    if dataset_path.exists():
                        ...
                        print(f"⚠️  Dataset 'dataset_{identifier}' not found in dataset_yolo_format/")
                
                if not valid_identifiers:
                    print("❌ No valid datasets found. Please check your dataset names/numbers.")
                    continue
                
                print(f"✅ Selected datasets: {', '.join(map(str, valid_identifiers))}")
    ...
    def evaluate_trained_models(self):
        ...
        if not EVALUATION_AVAILABLE:
            ...
            print("="*50)
            print("❌ Evaluation functionality is not available!")
            print("Please ensure you have:")
        ...
        if not result_dirs:
            print("❌ No training results with results.csv found!")
            print("Please train some models first using option 1.")
            input("\nPress Enter to continue...")
            return
    ...
    def display_visualization(self, viz_path, description):
        ...
        try:
            ...
            print("✅ Visualization opened in default viewer.")
            
        except subprocess.CalledProcessError:
            print(f"❌ Could not open visualization automatically.")
    ...
    def quick_results_summary(self):
        ...
        try:
            ...
            print(f"💥 Error running quick results summary: {e}")
    ...
    def remove_watermark(self):
        ...
        if not WATERMARK_REMOVER_AVAILABLE:
            print("❌ Watermark remover functionality is not available!")
            print("Please ensure you have:")
        ...
        print("🚀 Launching watermark remover...")
        
        try:
            watermark_remover_main()
        except Exception as e:
            print(f"💥 Error running watermark remover: {e}")
            print("Please check your setup and try again.")
    ...
    def split_videos(self):
        ...
        if not VIDEO_SPLITTER_AVAILABLE:
            print("❌ Video splitter functionality is not available!")
            print("Please ensure you have:")
        ...
        print("🚀 Launching video splitter...")
        print("💡 This tool splits videos larger than 100MB into smaller chunks")
        
        try:
            video_splitter_main()
        except Exception as e:
            print(f"💥 Error running video splitter: {e}")
            print("Please check your setup and try again.")
    ...
    def check_ffmpeg_availability(self):
        ...
        if local_ffmpeg.exists():
            if not local_ffprobe.exists():
                print(f"❌ FFmpeg found but ffprobe.exe is missing!")
                print(f"📥 Please download the complete FFmpeg package that includes both:")
                print(f"   • ffmpeg.exe ✅ (found)")
                print(f"   • ffprobe.exe ❌ (missing)")
                return False
            ...
            try:
                ...
                print(f"✅ FFmpeg and ffprobe found at: {Path('ffmpeg_bin').absolute()}")
                ...
                print(f"❌ Local FFmpeg/ffprobe found but not working: {e}")
                return False
    ...
    def show_ffmpeg_setup_instructions(self):
        ...
        print("📥 OPTION 1: Download Pre-built Binaries (Recommended)")
        ...
        print(f"\n⚡ Quick Install (if you have winget):")
        print("   winget install FFmpeg")
        
        print("\n💡 After setup, restart this program to use video splitting!")
        print("="*60)
    
    def run(self):
        """Main program loop"""
        print("🚀 Welcome to the Data Processing Toolkit!")
        ...
        while True:
            try:
                ...
                if choice == 1:
                    ...
                    print("\n👋 Thank you for using the Data Processing Toolkit!")
                    print("Goodbye! 🎉")
                    break
